import { openProject, type OpenProjectResult } from "../db/project.js";
...
export function whyRelated(repoPath: string, target: string, task: string): WhyRelatedResult {
  return whyRelatedBatch(repoPath, [target], task)[0] as WhyRelatedResult;
}

export function whyRelatedBatch(repoPath: string, targets: string[], task: string): WhyRelatedResult[] {
  const project = openProject(repoPath);
  try {
    return targets.map((target) => whyRelatedWithProject(project, target, task));
  } finally {
    project.db.close();
  }
}

function whyRelatedWithProject(project: OpenProjectResult, target: string, task: string): WhyRelatedResult {
